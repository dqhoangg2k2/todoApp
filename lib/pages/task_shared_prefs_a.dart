...
class TaskSharedPrefsA extends StatefulWidget {
  const TaskSharedPrefsA({super.key, required this.title});

  final String title;

  @override
  State<TaskSharedPrefsA> createState() => _TaskSharedPrefsAState();
}

class _TaskSharedPrefsAState extends State<TaskSharedPrefsA> {
  ...
  Future<void> _updateTask(String id, {String? text, bool? isDone}) async {
  ...
  Widget _taskItem(TaskModel task) {
    return GestureDetector(
      onTap: () => _updateTask(task.id ?? '', isDone: !(task.isDone ?? false)),
      child: Container(
